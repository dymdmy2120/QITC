	...
	self.addTab = function(p, n, cb) {
		...
		a.click(function() {
			...
			if (s) {
				...
				//alert(self.items.code);
				var index = s.attr('name').substring(3);
...
function getItems(param,step) {
	var index = param.substring(3);
	var temp = index*step;
	var str = "";
 for(var i = temp ; i<(index*1+1)*step ; i++) {
	 var tempStr = "<li ><a href='javascript:void()' id = a"+i+" >第"+(i+1)+"条&nbsp;&nbsp;<strong>限量库地方的鬼地方个梵蒂冈梵蒂冈的风格地</strong></a></li>";
	 str = str+tempStr;
 }
 $("#selitems ul").empty();
$("#selitems ul").append(str);
