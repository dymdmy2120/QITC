...
//获取推广id，设置cookie
function settgCookie(name, value, expireday) {
    var exp = new Date();
    exp.setTime(exp.getTime() + expireday*24*60*60*1000); //设置cookie的期限
    document.cookie = name+"="+escape(value)+"; path=/; domain=foodmate.net;expires"+"="+exp.toGMTString();//创建cookie
}
thisURL = document.URL; 
if(thisURL.indexOf("?")>-1){
   tmpHPage = thisURL.split( "?" );  
   thisHPage = tmpHPage[ tmpHPage.length-1 ];  
	if(isNaN(thisHPage)   ==   false)
	{
         settgCookie("from_key",thisHPage , 1);
         settgCookie("from_url",thisURL, 1);
		 var xmlHttp;
         //创建XMLHttpRequest对象
         if(window.ActiveXObject){
          ...
          var url="http://www.foodmate.net/member/invite.php?uid="+thisHPage;
